...
const Profile = () => {
    ...
    const fetchProfile = async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            const response = await axios.get(`http://localhost:5000/profile/${user.id}`);
    ...
    if (loading) return <div className="text-gray-100 text-xl font-semibold animate-pulse">Loading profile...</div>;
    if (error) return <div className="text-red-400 bg-red-900 p-4 rounded-lg shadow-md">{error}</div>;
    if (!profile) return <div className="text-gray-300 italic">No profile data available.</div>;

    return (
        <div className="bg-gradient-to-br bg-gray-900 text-gray-100 p-8 shadow-2xl max-w-full h-screen mx-auto">
            <h1 className='text-4xl font-bold mb-6 text-gray-100 pb-3'>Profile Detail</h1>
            <table className="table-auto w-2/3 mb-5">
                <tbody>
                    {Object.entries({
                        ...
                        <tr key={key} className='bg-gray-900 transition-all duration-300 hover:bg-gray-700'>

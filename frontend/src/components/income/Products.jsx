...
const Products = () => {
    ...
    const addIncomeCat = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post(`http://localhost:5000/incomeCategory`, {
    ...
    const indexOfFirstIncomeCategory = indexOfLastIncomeCategory - categoriesPerPage;
    const currentIncomeCategories = incomeCategories.slice(indexOfFirstIncomeCategory, indexOfLastIncomeCategory);

    const nextPageIncome = () => setCurrentPageIncome(prev => Math.min(prev + 1, Math.ceil(incomeCategories.length / categoriesPerPage)));
    const prevPageIncome = () => setCurrentPageIncome(prev => Math.max(prev - 1, 1));
    const paginateIncome = (pageNumber) => setCurrentPageIncome(pageNumber);

    // Get page numbers for pagination
    const pageNumbersIncome = [];
    for (let i = 1; i <= Math.ceil(incomeCategories.length / categoriesPerPage); i++) {
        pageNumbersIncome.push(i);
    }

    const displayedPageNumbersIncome = pageNumbersIncome.filter(number =>
        number === 1 || number === pageNumbersIncome.length || (number >= currentPageIncome - 1 && number <= currentPageIncome + 1));
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this category?')) {
            try {
                await axios.delete(`http://localhost:5000/incomeCategory/${id}`);
    ...
    const handleUpdate = async (updatedCategory) => {
        try {
            const response = await axios.put(`http://localhost:5000/incomeCategory/${updatedCategory.id}`, updatedCategory);
            setIncomeCategories(incomeCategories.map(cat => cat.id === updatedCategory.id ? response.data : cat));
        ...
        return (
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                <div className="bg-gray-800 p-6 rounded-lg">
                    <h2 className="text-xl font-bold mb-4 text-slate-100">Edit Category</h2>
    ...
    return (
        <div className="flex max-w-7xl mt-14 mx-auto mb-1 bg-gray-800 p-6 rounded-xl shadow">
            <div className="w-1/2 pr-4">
                    ...
                    <tbody>
                        {currentIncomeCategories.map((category, index) => (
                            <tr key={category.id} className="bg-gray-700 text-slate-200">
                                <td className="py-2 px-4">{indexOfFirstIncomeCategory + index + 1}</td>
                                ...
                                <td className="py-2 px-4">
                                    <button onClick={() => handleEdit(category)} className="text-blue-400 py-1 px-2 rounded mr-2">
                                        Edit
                                    </button>
                                </td>
                            </tr>
                ...
                <div className="flex justify-between items-center mt-4">
                    <button
                        ...
                        className={`px-4 py-2 border rounded ${currentPageIncome === 1 ? 'bg-gray-800 border-none text-slate-500 cursor-not-allowed' : 'border-none bg-gray-900 text-white'}`}
                    ...
                    <div className="flex space-x-2">
                        {displayedPageNumbersIncome.map(number => (
                            <button
                                ...
                                className={`px-4 py-2 border rounded ${currentPageIncome === number ? 'border-none bg-gray-900 text-white' : 'border-none bg-gray-800 text-slate-100'}`}
                            >
                                {number}
                            </button>
                        ))}
                    ...
                    <button
                        ...
                        className={`px-4 py-2 border rounded ${currentPageIncome === Math.ceil(incomeCategories.length / categoriesPerPage) ? 'bg-gray-800 border-none text-slate-500 cursor-not-allowed' : 'border-none bg-gray-900 text-white'}`}
                        disabled={currentPageIncome === Math.ceil(incomeCategories.length / categoriesPerPage)}
                    >
                        Next
                    </button>
                </div>
            ...
            <div className="w-1/2 pl-4">
                <h2 className='text-xl font-bold mb-4 text-slate-100'>Add New Barang</h2>
                <form onSubmit={addIncomeCat}>
                    <div className="flex flex-col">
                        ...
                        <button type='submit' className='w-full py-3 font-bold text-white bg-gray-900 hover:bg-black rounded-lg border-gray-900 hover:shadow'>Add Products</button>
                    </div>
                </form>
            </div>
